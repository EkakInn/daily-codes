...
Defining A Loggine
"""
logger.add(f"{logs_folder}Database_Argus.log", level="INFO", rotation="100 MB")
...
class ArgusDB:
    ...
    def check_env_data(self) -> bool:
        ...
        '''
        env_values = (self.db_name, self.db_user,
    ...
    def create_connection(self):
        ...
        try:
            ...
            logger.debug(F"Issue in Creation Connection {e}")
    ...
    def multiple_query(self, all_query: list) -> bool:
        ...
        if connection != False:
            try:
                ...
                print(
                    "Error in transaction, reverting all changes using rollback ", error)
                logger.debug(f"Issue is E {e}")
                connection.rollback()
    ...
    def create_new_user(self, new_user_email, new_user_password, new_user_name, role, customer_id):
        status_insertion = self.insert_new_user(
            new_user_email, new_user_password, customer_id)
    ...
    def get_all_incident(self, customer_id):
        """Get all Users Email , Users HashPassword, 
        ...
        try:
            ...
            query = F'SELECT priority,incident_id,category,date,assignee,location,status,assigned_by,mid FROM argus.\"{table_name}\" ; '
            return self.get_all(query)
        except Exception as E:
            logger.exception("Error in user Email")
            return False
    ...
    def get_today_incident(self, customer_id, date):
        ...
        try:
            ...
            logger.info(
                f'_Get priority,incident_id,category,date,assignee,location,status,assigned_by,location Where Table: {table_name} ')
            query = F'SELECT priority,incident_id,category,date,assignee,location,status,assigned_by FROM argus.\"{table_name}\" where date=\"{date}\"; '
            return self.get_query(query)
        except Exception as E:
            logger.exception("Error in user Email")
            return False
    ...
    def insert_new_incident(self, customer_id, incident_id, priority, category, date, location, status, user_id,
        ...
        try:
            ...
            if q_status:
                ...
                query2 = '''insert into "argus"."{}" (incident_id,incident_status) values (%s, %s)'''.format(
                    table2)
                if comment:
